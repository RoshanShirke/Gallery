window.onload = function () {
    ...
    fetch(`https://api.unsplash.com/photos?per_page=25&query=${search_params.get('q')}&client_id=${API_KEY}`).then(convert_to_json)
    .then(function(data){
        generateCards(data);
...
function generateCards(data) {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        card.style.backgroundColor = `url(${single_item.urls.thumb})`;
